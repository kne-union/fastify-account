...
module.exports = fp(async function(fastify, options) {
  ...
  options = merge({
    ...
    dbTableNamePrefix: 't_account_', isTest: false, jwt: {
  ...
  fastify.register(require('@kne/fastify-namespace'), {
    ...
    })], ['services', path.resolve(__dirname, './libs/services')], ['controllers', path.resolve(__dirname, './libs/controllers')], ['authenticate', {
      user: async request => {
        ...
        request.tenantInfo = await fastify.account.services.tenantUser.getTenantUserByUserId({
          userInfo: request.userInfo, appName: request.appName
        });
        await fastify.account.services.requestLog.addRequestLog({
          userInfo: request.userInfo,
  ...
  name: 'fastify-account', dependencies: ['fastify-sequelize']
});
